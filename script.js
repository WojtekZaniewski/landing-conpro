...
async function submitLead(payload) {
  ...
  const response = await fetch(FORM_SUBMIT_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      _subject: "Nowy lead - Meta Ads - Odszkodowania",
      _template: "table",
      _captcha: "false",
    }),
  });
...
async function submitNewsletterLead(email) {
  ...
  const response = await fetch(FORM_SUBMIT_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      sourcePage: window.location.href,
      submittedAt: new Date().toISOString(),
      _subject: "Newsletter lead - checklista odzyskiwania odszkodowania",
      _template: "table",
      _captcha: "false",
    }),
  });
...
form.addEventListener("submit", async (event) => {
  ...
  if (!payload.rodoConsent) {
    setStatus("Musisz zaznaczyc zgode RODO.", "err");
    return;
  }
...
if (newsletterModal) {
  ...
  newsletterForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      submitButton.textContent = "Wyślij i odbierz wskazówki";
    }
  });
}
